...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if(isset($_GET[VAR_PATHINFO])) { // 判断URL里面是否有兼容模式参数
        ...
        // 获取模块名称
        define('MODULE_NAME', defined('BIND_MODULE')? BIND_MODULE : self::getModule(VAR_MODULE));
        ...
        if(!defined('__APP__')){
	        ...
	        if($urlMode == URL_COMPAT ){// 兼容模式判断
	            define('PHP_FILE',_PHP_FILE_.'?'.VAR_PATHINFO.'=');
        ...
        // 获取控制器的命名空间（路径）
        define('CONTROLLER_PATH',   self::getSpace(VAR_ADDON,$urlCase));
        // 获取控制器和操作名
        define('CONTROLLER_NAME',   defined('BIND_CONTROLLER')? BIND_CONTROLLER : self::getController(VAR_CONTROLLER,$urlCase));
        define('ACTION_NAME',       defined('BIND_ACTION')? BIND_ACTION : self::getAction(VAR_ACTION,$urlCase));
