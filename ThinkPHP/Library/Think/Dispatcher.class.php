...
class Dispatcher
{
    ...
    public static function dispatch()
    {
        ...
        if (MODULE_NAME && !in_array_case(MODULE_NAME, C('MODULE_DENY_LIST')) && is_dir(APP_PATH . MODULE_NAME)) {
            ...
            // 加载应用模式对应的配置文件
            if ('common' != APP_MODE && is_file(MODULE_PATH . 'Conf/'.APP_ENV_PATH.'config_' . APP_MODE . CONF_EXT)) {
                C(load_config(MODULE_PATH . 'Conf/'.APP_ENV_PATH.'config_' . APP_MODE . CONF_EXT));
            }

            // 当前应用状态对应的配置文件
            if (APP_STATUS && is_file(MODULE_PATH . 'Conf/'. APP_ENV_PATH . APP_STATUS . CONF_EXT)) {
                C(load_config(MODULE_PATH . 'Conf/'. APP_ENV_PATH . APP_STATUS . CONF_EXT));
            }
